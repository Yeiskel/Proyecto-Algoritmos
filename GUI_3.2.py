...
def contador_fichas(target:int,tablero:int)->int:
    suma=0
    for f in range(9):
        for c in range(9):
...
def otra_partida():                 
    ...
    while not over:
        ...
        #Boton de Jugar
        boton("Jugar de nuevo",620,250,250,50,BLACK, GREEN,juego_completo)

        #Boton de Salir
        boton("Salir", 700, 350, 100, 50, BLACK, GREEN,quit)
...
def boton(mensaje:str, x:int, y:int, ancho:int, alto:int, color_activo:tuple, color_inactivo:tuple, comando=None):  
    ...
    textSurf, textrect = cuadros_texto(mensaje, fuente, WHITE)
    textrect.center = ((x +(ancho//2)), y + (alto//2))
    screen.blit(textSurf,textrect)          
def cuadros_texto(texto:str, fuente, color):
    textscreen = fuente.render(texto, True, color)
...
def dibujar_cuadros_texto(mensaje:str, x:int, y:int, ancho:int, alto:int, fuente, color:tuple):
    textSurf, textrect = cuadros_texto(mensaje, fuente, color)
    textrect.center = ((x +(ancho//2)), y + (alto//2))
    screen.blit(textSurf,textrect)  
...
def jugada_invalida(invalida:bool):
    if invalida== True:
            dibujar_cuadros_texto("Jugada invalida", 700, 150, 100, 100, fuente, RED)
...
def resultados(tablero:list)->str:
    ...
    if fichas_blancas > fichas_negras:
        print("\nGana el jugador 1")
        #Mensaje de victoria
        dibujar_cuadros_texto("Gana el jugador 1", 700, 400, 100, 100, fuente, BLACK)

    elif fichas_negras > fichas_blancas:
        print("\nGana el jugador 2")
        #Mensaje de victoria
        dibujar_cuadros_texto("Gana el jugador 2", 700, 400, 100, 100, fuente, WHITE)
    else:
        print("\nEmpate")
    pygame.display.update()
#-----------------JUEGO------------------------#
