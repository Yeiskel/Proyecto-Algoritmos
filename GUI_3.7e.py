...
def simulador(tablero:list, turno:int, fila:int, columna:int)->bool:
    ...
    while f<8:
        ...
        f+=1
    return estado
def mov_posibles(copia_tablero:list, turno:int, mensaje:bool)->list:
    ...
    for fila in range(1,9):
        for columna in range(1,9):
            ...
            if temp==True:
                ...
                casillas.append(columna)
                opciones.append(casillas)
                if mensaje==True:
                    print(f"Tablero[{fila}][{columna}]={temp}")
    casillas=[]
    casillas.append(mov_posibles)
    casillas.append(mov_posibles)
    opciones.insert(0,casillas)  
    if mensaje==True:
        ...
        for event in pygame.event.get():   
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_a: #columnas
                    ...
                    mov= mov_posibles(tablero, turno, True)
